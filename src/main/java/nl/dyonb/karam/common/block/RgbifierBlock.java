...
public class RgbifierBlock extends Block implements BlockEntityProvider {
    public RgbifierBlock() {
        super(FabricBlockSettings.of(Material.WOOD).breakByTool(FabricToolTags.AXES, ToolMaterials.WOOD.getMiningLevel())
                .strength(1F, 1F).sounds(BlockSoundGroup.WOOD));
    }

    @Override
    public BlockEntity createBlockEntity(BlockView world) {
        return new RgbifierBlockEntity();
    }
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if (blockEntity instanceof RgbifierBlockEntity) {
                ItemScatterer.spawn(world, pos, (RgbifierBlockEntity)blockEntity);
                // update comparators
